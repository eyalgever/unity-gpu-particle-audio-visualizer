...
Shader "Custom/Particles" {
	...
	CGINCLUDE
			...
			void geom (point v2f p[1], inout TriangleStream<fs_input> triStream)
			{
				...
				float3 look =  _WorldSpaceCameraPos - mul(unity_ObjectToWorld , p[0].position);
				look.y = 0;
				look = normalize(look);
				...
				float4 v[4];
				v[0] = float4( mul(unity_ObjectToWorld , p[0].position) + halfS * right - halfS * up, 1.0f);
				v[1] = float4( mul(unity_ObjectToWorld , p[0].position) + halfS * right + halfS * up, 1.0f);
				v[2] = float4( mul(unity_ObjectToWorld , p[0].position) - halfS * right - halfS * up, 1.0f);
				v[3] = float4( mul(unity_ObjectToWorld , p[0].position) - halfS * right + halfS * up, 1.0f);
